"""Minimal Fourier layers for FNO baselines."""
...
class SpectralConv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, modes: int):
        super().__init__(); self.in_channels=in_channels; self.out_channels=out_channels; self.modes=modes
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.shape; x_ft = torch.fft.rfft2(x)
        out_ft = torch.zeros(b, self.out_channels, h, w//2+1, dtype=torch.cfloat, device=x.device)
        mh = min(self.modes, h); mw = min(self.modes, w//2+1)
        out_ft[:, :, :mh, :mw] = torch.einsum('bixy,ioxy->boxy', x_ft[:, :, :mh, :mw], self.weights[:, :, :mh, :mw])
...
class SpectralConv3d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b,c,d,h,w=x.shape; x_ft=torch.fft.rfftn(x, dim=(-3,-2,-1))
        out_ft=torch.zeros(b,self.out_channels,d,h,w//2+1,dtype=torch.cfloat,device=x.device)
        md=min(self.modes,d); mh=min(self.modes,h); mw=min(self.modes,w//2+1)
